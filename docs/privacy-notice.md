...
Feedback is quarantined. A person may separately consent to consideration of that feedback for research, but consent alone does not place it in a dataset or export: an Analyst must independently adjudicate the report first. Governed exports exclude feedback comments, account identifiers and original URLs, and use only redacted link displays and pseudonymous record identifiers.

The application can be installed through supported browser or device controls. Its install manifest and icons do not add an offline scan cache or background target retrieval; network and retention behaviour remains the same as in the browser.
